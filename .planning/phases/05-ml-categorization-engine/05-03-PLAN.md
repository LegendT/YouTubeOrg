...
export async function runMLCategorization(): Promise<RunMLCategorizationResult> {
  try {
    ...
    await db
      ...
      .where(
        eq(
          mlCategorizations.videoId,
          videosToProcess.map((v) => v.id)[0]
        )
      );
...
Check: Database insert uses mlCategorizations table from schema
  ...
  <done>
ML types defined with MLCategorizationResult and RunMLCategorizationResult interfaces, server action runMLCategorization implemented with categorization engine orchestration, database persistence with delete+insert pattern for re-runs, statistics calculation (high/medium/low counts), getMLCategorizationForVideo and getMLCategorizationResults query functions, proper error handling and engine cleanup.
  </done>
</task>
