...
must_haves:
  truths:
    ...
    - "Finalization sets finalizedAt timestamp on the analysisSessions record"
  artifacts:
    ...
    - path: "src/app/actions/analysis.ts"
      provides: "finalizeConsolidation server action that sets finalizedAt on analysisSessions"
      exports: ["finalizeConsolidation"]
  key_links:
    - from: "src/components/analysis/final-review.tsx"
      to: "src/app/actions/analysis.ts"
      via: "finalizeConsolidation server action"
      pattern: "finalizeConsolidation"
    - from: "src/app/actions/analysis.ts"
      to: "src/lib/db/schema.ts"
      via: "Update analysisSessions.finalizedAt"
      pattern: "finalizedAt"
...
<task type="auto">
  <name>Task 1: Create final review screen and finalizeConsolidation server action</name>
  <files>src/components/analysis/final-review.tsx, src/app/actions/analysis.ts</files>
  <action>
1. **Add finalizeConsolidation server action** to src/app/actions/analysis.ts:

```typescript
export async function finalizeConsolidation(): Promise<{ success: boolean; categoryCount?: number; errors?: string[] }> {
  // 1. Get the latest analysis session
  const sessions = await db.select().from(analysisSessions).orderBy(desc(analysisSessions.createdAt)).limit(1)
  ...
  const approved = await db.select().from(consolidationProposals)
    .where(and(
  ...
  for (const proposal of approved) {
    if ((proposal.uniqueVideoCount ?? proposal.totalVideos) > 4500) {
      return { success: false, errors: [`Category "${proposal.categoryName}" exceeds 4,500 video limit`] }
    }
  }
...
This marks the session as finalized by setting the finalizedAt timestamp on the analysisSessions record. The approved proposal structure becomes the target category structure for Phase 3 (Category Management) and Phase 8 (Batch Sync).

NOTE: This does NOT sync to YouTube -- that's Phase 8. For Phase 2, "Execute consolidation" means "lock in this approved structure as final."

2. **Create src/components/analysis/final-review.tsx** as Client Component implementing the pre-execution comprehensive review.
...
- On click: calls finalizeConsolidation() server action, then calls onExecute callback
- Loading state while executing
- On success: show "Consolidation structure saved. {X} categories ready for Phase 3."
...
Check finalizedAt used:
```bash
grep "finalizedAt" src/app/actions/analysis.ts
```
  </verify>
  <done>
FinalReview shows summary statistics, detailed change list, before/after impact visualization, and "Execute consolidation" button. finalizeConsolidation server action validates approved proposals and sets finalizedAt on the analysisSessions record. Dialog uses ScrollArea for long content. Button disabled when no approved proposals. TypeScript compiles.
  </done>
</task>
...
14. Finalization sets finalizedAt on analysisSessions
  </what-built>
  <how-to-verify>
...
8. **Category detail:**
   ...
   - See validation badge with progress bar (showing X / 5,000 videos)
...
13. **Final review:**
    ...
    - Verify it sets finalizedAt (check DB: `SELECT finalized_at FROM analysis_sessions ORDER BY id DESC LIMIT 1;`)
    - See success confirmation: "Consolidation structure saved. X categories ready for Phase 3."
...
5. **finalizeConsolidation:** Sets finalizedAt on analysisSessions record
6. **Keyboard navigation works:** Arrow keys navigate, Enter selects
7. **Batch operations work:** Multi-select + batch approve/reject
8. **Split wizard works:** 3-step dialog creates new proposals
9. **Progress tracking updates:** Counter reflects reviewed state
10. **Human verification passed:** Complete workflow confirmed
...
- "Execute consolidation" calls finalizeConsolidation which sets finalizedAt on analysisSessions
