...
must_haves:
  truths:
    ...
    - "User sees color-coded validation badge: Green (<3000), Yellow (3000-4500), Red (>4500) with progress bar showing count out of 5,000"
  ...
  artifacts:
    ...
    - path: "src/components/analysis/validation-badge.tsx"
      provides: "Category size validation indicator with progress bar (denominator 5,000, danger at 4,500)"
    ...
    - path: "src/app/actions/analysis.ts"
      provides: "getCategoryDetail server action for on-demand detail data fetching"
      exports: ["getCategoryDetail"]
  key_links:
    ...
    - from: "src/components/analysis/analysis-dashboard.tsx"
      ...
      via: "getCategoryDetail for on-demand detail data"
      pattern: "getCategoryDetail"
---
...
- Calculate status thresholds:
  - 'safe': videoCount < 3,000 (green)
  - 'warning': videoCount >= 3,000 AND videoCount <= 4,500 (yellow)
  - 'danger': videoCount > 4,500 (red)
...
- Below badge: Progress component showing videoCount / 5,000 ratio
  - The denominator is 5,000 (YouTube's absolute maximum playlist size limit)
  - The danger threshold is 4,500 (safe operating margin, 500 videos below YouTube's hard limit)
  - Progress bar color matches status (green/yellow/red via CSS classes)
  - Label: "{videoCount.toLocaleString()} / 5,000 videos"
- If danger (>4,500): add inline warning text "Exceeds safe limit of 4,500 videos (YouTube maximum: 5,000)"
- If warning (3,000-4,500): add inline caution text "Approaching limit"
...
Expected: ConfidenceBadge, ValidationBadge, ProposalActions exported.
  ...
  <done>
ConfidenceBadge renders color-coded HIGH/MEDIUM/LOW with reasoning text. ValidationBadge renders Green/Yellow/Red with progress bar showing videoCount / 5,000 (danger threshold at 4,500, YouTube max at 5,000). ProposalActions renders approve/reject buttons with natural language summary. All use shadcn/ui components. TypeScript compiles.
  </done>
</task>
...
```
  ...
  <done>
VideoListPaginated uses @tanstack/react-table with pagination (50/100/250/all), search by title, filter by source playlist, YouTube preview links. CategoryDetail composes confidence badge, validation badge (showing count/5,000 with 4,500 danger threshold), metrics, video list, and proposal actions in a scrollable panel. AnalysisDashboard wires CategoryDetail into right panel with on-demand data fetching via getCategoryDetail server action. TypeScript compiles.
  </done>
</task>
...
7. **Validation badges:** Color-coded Green/Yellow/Red with progress bar showing count/5,000 and danger at 4,500
8. **Actions at bottom:** Approve/reject buttons in detail panel
9. **Dashboard wired:** CategoryDetail renders in right panel
10. **getCategoryDetail server action:** Exists and returns CategoryMetrics + VideoDetail[]
...
- Validation badge shows color-coded size with progress bar (denominator 5,000, danger at 4,500)
- Approve/reject buttons work at bottom of detail panel
- getCategoryDetail server action provides on-demand data fetching
